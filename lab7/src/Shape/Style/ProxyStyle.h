...
public:
    ...
    [[nodiscard]] std::optional<bool> IsEnabled() const override
    {
        ...
        for (const auto &pair: m_shapes)
        {
            const auto styleEnabled = m_isOutline
                                      ? pair.second->GetOutlineStyle().IsEnabled()
                                      : pair.second->GetFillStyle().IsEnabled();
    ...
    void Enable(bool enabled) override
    {
        ...
        for (const auto &pair: m_shapes)
        {
            if (m_isOutline)
            {
                pair.second->GetOutlineStyle().Enable(enabled);
            }
            else
            {
                pair.second->GetFillStyle().Enable(enabled);
            }
        }
    }
    ...
    [[nodiscard]] std::optional<RGBAColor> GetColor() const override
    {
        ...
        for (const auto &pair: m_shapes)
        {
            const auto styleColor =
                    m_isOutline ? pair.second->GetOutlineStyle().GetColor() : pair.second->GetFillStyle().GetColor();
    ...
    void SetColor(RGBAColor color) override
    {
        ...
        for (const auto &pair: m_shapes)
        {
            if (m_isOutline)
            {
                pair.second->GetOutlineStyle().SetColor(color);
            }
            else
            {
                pair.second->GetFillStyle().SetColor(color);
            }
        }
    }
